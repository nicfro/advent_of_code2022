...
with open("day_11_Monkey_in_the_Middle/input.txt") as f:
    lines = f.readlines()
...
def monkey_business(monkey, monkeys):
    while monkeys[monkey]["items"]:
        ...
        worry = eval("".join([item if x == "old" else x for x in operations]))
        test = worry % monkeys[monkey]["test"] == 0
        to_monkey = monkeys[monkey][test]
        monkeys[to_monkey]["items"].append(str(worry % monkey_divisor))
        monkeys[monkey]["inspections"] += 1


monkey_queue = list(sorted(monkeys.keys()))
monkey_divisor = math.prod([monkeys[x]["test"] for x in monkey_queue])
